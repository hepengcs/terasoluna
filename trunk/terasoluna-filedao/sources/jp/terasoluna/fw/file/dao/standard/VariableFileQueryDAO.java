 ...
 * <p>
 * 可変長ファイルからファイル行オブジェクトを生成する<code>FileLineIterator</code>
 * を生成するためのクラスである。引数にはデータを読み取る可変長ファイルのパスと
 * ファイル行オブジェクトクラスを設定すること。
 * </p>
 * 
 * 行オブジェクトに設定出来るアノテーションの説明は{@link VariableFileLineIterator}
 * のJavaDocを参考して下さい。
 * 
 */
public class VariableFileQueryDAO extends AbstractFileQueryDAO {
    ...
    public <T> FileLineIterator<T> execute(String fileName, Class<T> clazz) {
        ...
        VariableFileLineIterator<T> fileLineIterator =
            new VariableFileLineIterator<T>(fileName, clazz,
                    getColumnParserMap());

        //FileLineIteratorを初期化する。
